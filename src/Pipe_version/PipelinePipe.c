...
void Pipeline_execute(Pipeline* this) {
    ...
    PipelineChannel* prev_channel = NULL;
    while (stage) {
    ...
    // Close all channels
    stage = this->head_stage;
    while (stage) {
        if (stage->input_channel) {
            close(((PipelineChannel*) stage->input_channel)->fd[0]);
            close(((PipelineChannel*) stage->input_channel)->fd[1]);
        }
        if (stage->output_channel) {
            close(((PipelineChannel*) stage->output_channel)->fd[0]);
            close(((PipelineChannel*) stage->output_channel)->fd[1]);
        }
        stage = stage->next_stage;
    }
    free(prev_channel);
...
bool Pipeline_send(void* channel, void* buffer, size_t size) {
    ...
    PipelineChannel* pipeline_channel = (PipelineChannel*) channel;
    if (write(pipeline_channel->fd[1], buffer, size) == -1) {
        perror("write");
        exit(EXIT_FAILURE);
    }
    return true;
...
bool Pipeline_receive(void* channel, void* buffer, size_t size) {
    ...
    PipelineChannel* pipeline_channel = (PipelineChannel*) channel;
    if (read(pipeline_channel->fd[0], buffer, size) == -1) {
        perror("read");
        exit(EXIT_FAILURE);
    }
    return true;
